...
const isPersonal = computed(() => {
  return route.path.match('^/publishing/book-example/settings$')
})

const isBook = computed(() => {
  return route.path.match('^/publishing/book-example/settings/book$')
})

const isResearch = computed(() => {
  return route.path.startsWith('/publishing/book-example/settings/research')
})
...
<template>
  <div class="dark:bg-muted-900 min-h-screen bg-white">
    <NavigationTop title="Settings" close-to="/publishing/book/editor" />
          ...
          <div class="md:col-span-3 lg:col-span-3">
            ...
            <div class="border-muted-200 dark:border-muted-800 h-full border-r">
              <ul class="xs:flex xs:gap-4 -me-0.5 font-sans">
                ...
                <li>
                  <NuxtLink
                    to="/publishing/book/settings/book"
                    class="font-heading xs:border-b-[3px] flex w-full cursor-pointer py-2 text-sm md:border-r-[3px]"
                    :class="
                      isBook
                        ? 'text-muted-800 dark:text-muted-100 border-primary-500'
                ...
                <li>
                  <NuxtLink
                    to="/publishing/book/settings/research"
                    class="font-heading xs:border-b-[3px] flex w-full cursor-pointer py-2 text-sm md:border-r-[3px]"
                    :class="
                      isResearch
                        ? 'text-muted-800 dark:text-muted-100 border-primary-500'
