...
const selectCurrentResearch = async (id: number) => {
    console.log(id)
 let res = await getResearchById(id)
 console.log(res)
  router.push(`/research/projects/${id}`)
}

const selectedProject = ref<NonNullable<typeof researchProjects.value>['data'][0]>()
</script>
  ...
  <div class="mx-auto max-w-2xl">
    ...
    <div v-else>
      <div class="my-6 text-center">
        ...
        <div class="mx-auto mt-6 flex max-w-xs justify-center">
          <BaseAutocomplete
            ...
            :display-value="(item) => item.main_category"
            rounded="lg"
            icon="lucide:search"
            placeholder="Search..."
          ...
          >
            ...
            <template #item="{ item, selected }">
              <div
              @click=selectCurrentResearch(item.id)
                class="block"
              >
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        >
          <div
            v-for="(item, r) in researchProjects.slice(0, 4)"
            :key="r"
            class="group block"
            @click=selectCurrentResearch(item.id)
          >
            ...
            >
              <div class="mb-6 flex gap-2">
                <BaseAvatar
                  ...
                  size="sm"
                  rounded="full"
                  :data-nui-tooltip="item.name ?? item.main_category"
