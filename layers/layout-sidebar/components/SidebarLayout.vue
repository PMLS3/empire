<script setup lang="ts">
  ...
  const toolbarEnabled = computed(() => {
    return (
      (app.tairo?.sidebar?.toolbar?.enabled as boolean) !== false &&
      props.toolbar !== false
    )
  })
  ...
  const wrapperClass = computed(() => {
    ...
    if (!sidebarEnabled.value) {
      ...
      return "bg-muted-100 dark:bg-muted-900 relative min-h-screen w-full overflow-x-hidden px-4 transition-all duration-300 xl:px-10"
        ...
        return "bg-muted-100 dark:bg-muted-900 relative min-h-screen w-full overflow-x-hidden pl-4 transition-all duration-300 xl:pl-10"
      }
    }
    ...
    if(isAssistantOpen.value) {
      list = [
      "bg-muted-100 dark:bg-muted-900 relative min-h-screen w-full overflow-x-hidden pl-4 transition-all duration-300 xl:pl-10",
    ]
    }
    ...
    <div :class="wrapperClass">
      <div
        :class="[
          ...
          !props.horizontalScroll && 'mx-auto w-full',
          isAssistantOpen ? 'w-full' : 'max-w-7xl'
        ]"
      >
        <slot name="toolbar">
          <SidebarToolbar
